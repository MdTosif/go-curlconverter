...
func TestGenerate(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			contains: "HTTPoison.get!",
		},
		{
			name:     "POST with data",
			curl:     "curl -X POST -d 'key=value' http://example.com",
			contains: "HTTPoison.post!",
		},
	}
...
func TestFixtures(t *testing.T) {
	...
	elixirDir := filepath.Join(fixturesDir, "elixir")

	entries, err := os.ReadDir(elixirDir)
	if err != nil {
		t.Skipf("cannot read fixtures directory: %v", err)
	}

	for _, entry := range entries {
		if entry.IsDir() || filepath.Ext(entry.Name()) != ".exs" {
			continue
		}
		base := entry.Name()[:len(entry.Name())-len(".exs")]
		t.Run(base, func(t *testing.T) {
			cmdPath := filepath.Join(commandDir, base+".sh")
			elixirPath := filepath.Join(elixirDir, entry.Name())

			cmd, err := os.ReadFile(cmdPath)
			if err != nil {
				t.Fatalf("cannot read command file: %v", err)
			}
			expected, err := os.ReadFile(elixirPath)
			if err != nil {
