...
func TestFixtures(t *testing.T) {
	...
	javaOkhttpDir := filepath.Join(fixturesDir, "java-okhttp")

	entries, err := os.ReadDir(javaOkhttpDir)
	if err != nil {
	...
	for _, entry := range entries {
		...
		t.Run(base, func(t *testing.T) {
			...
			javaOkhttpPath := filepath.Join(javaOkhttpDir, entry.Name())

			cmd, err := os.ReadFile(cmdPath)
			if err != nil {
				t.Fatalf("cannot read command file: %v", err)
			}
			expected, err := os.ReadFile(javaOkhttpPath)
			if err != nil {
