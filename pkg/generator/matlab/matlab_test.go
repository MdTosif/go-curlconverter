...
func TestFixtures(t *testing.T) {
	...
	matlabDir := filepath.Join(fixturesDir, "matlab")

	entries, err := os.ReadDir(matlabDir)
	if err != nil {
	...
	for _, entry := range entries {
		...
		t.Run(base, func(t *testing.T) {
			...
			matlabPath := filepath.Join(matlabDir, entry.Name())

			cmd, err := os.ReadFile(cmdPath)
			if err != nil {
				t.Fatalf("cannot read command file: %v", err)
			}
			expected, err := os.ReadFile(matlabPath)
			if err != nil {
