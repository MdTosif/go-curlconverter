...
func TestFixtures(t *testing.T) {
	...
	clojureDir := filepath.Join(fixturesDir, "clojure")

	entries, err := os.ReadDir(clojureDir)
	if err != nil {
		t.Skipf("cannot read fixtures directory: %v", err)
	}

	for _, entry := range entries {
		if entry.IsDir() || filepath.Ext(entry.Name()) != ".clj" {
			continue
		}
		base := entry.Name()[:len(entry.Name())-len(".clj")]
		t.Run(base, func(t *testing.T) {
			cmdPath := filepath.Join(commandDir, base+".sh")
			clojurePath := filepath.Join(clojureDir, entry.Name())

			cmd, err := os.ReadFile(cmdPath)
			if err != nil {
				t.Fatalf("cannot read command file: %v", err)
			}
			expected, err := os.ReadFile(clojurePath)
			if err != nil {
