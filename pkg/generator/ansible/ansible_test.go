...
func TestGenerateMatchesAnsibleFixtures(t *testing.T) {
	t.Parallel()

	entries, err := os.ReadDir(filepath.Join("..", "..", "..", "test", "fixtures", "ansible"))
	if err != nil {
		t.Fatalf("read ansible fixtures: %v", err)
	}
	...
	for _, entry := range entries {
		...
		t.Run(entry.Name(), func(t *testing.T) {
			...
			base := entry.Name()[:len(entry.Name())-len(".yml")]
			cmdPath := filepath.Join("..", "..", "..", "test", "fixtures", "curl_commands", base+".sh")
			expectedPath := filepath.Join("..", "..", "..", "test", "fixtures", "ansible", entry.Name())
