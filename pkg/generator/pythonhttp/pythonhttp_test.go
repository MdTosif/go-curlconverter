...
func TestFixtures(t *testing.T) {
	...
	pythonHttpDir := filepath.Join(fixturesDir, "python-http")

	entries, err := os.ReadDir(pythonHttpDir)
	if err != nil {
	...
	for _, entry := range entries {
		...
		t.Run(base, func(t *testing.T) {
			...
			pythonHttpPath := filepath.Join(pythonHttpDir, entry.Name())

			cmd, err := os.ReadFile(cmdPath)
			if err != nil {
				t.Fatalf("cannot read command file: %v", err)
			}
			expected, err := os.ReadFile(pythonHttpPath)
			if err != nil {
