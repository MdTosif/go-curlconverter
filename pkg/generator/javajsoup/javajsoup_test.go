...
func TestFixtures(t *testing.T) {
	...
	jsoupDir := filepath.Join(fixturesDir, "java_jsoup")

	entries, err := os.ReadDir(jsoupDir)
	if err != nil {
	...
	for _, entry := range entries {
		...
		t.Run(base, func(t *testing.T) {
			...
			jsoupPath := filepath.Join(jsoupDir, entry.Name())

			cmd, err := os.ReadFile(cmdPath)
			if err != nil {
				t.Fatalf("cannot read command file: %v", err)
			}
			expected, err := os.ReadFile(jsoupPath)
			if err != nil {
