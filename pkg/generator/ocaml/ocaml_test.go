...
func TestFixtures(t *testing.T) {
	...
	ocamlDir := filepath.Join(fixturesDir, "ocaml")

	entries, err := os.ReadDir(ocamlDir)
	if err != nil {
		t.Skipf("cannot read fixtures directory: %v", err)
	}

	for _, entry := range entries {
		if entry.IsDir() || filepath.Ext(entry.Name()) != ".ml" {
			continue
		}
		base := entry.Name()[:len(entry.Name())-len(".ml")]
		t.Run(base, func(t *testing.T) {
			cmdPath := filepath.Join(commandDir, base+".sh")
			ocamlPath := filepath.Join(ocamlDir, entry.Name())

			cmd, err := os.ReadFile(cmdPath)
			if err != nil {
				t.Fatalf("cannot read command file: %v", err)
			}
			expected, err := os.ReadFile(ocamlPath)
			if err != nil {
