...
func TestFixtures(t *testing.T) {
	...
	pythonRequestsDir := filepath.Join(fixturesDir, "python")

	entries, err := os.ReadDir(pythonRequestsDir)
	if err != nil {
	...
	for _, entry := range entries {
		...
		t.Run(base, func(t *testing.T) {
			...
			pythonRequestsPath := filepath.Join(pythonRequestsDir, entry.Name())

			cmd, err := os.ReadFile(cmdPath)
			if err != nil {
				t.Fatalf("cannot read command file: %v", err)
			}
			expected, err := os.ReadFile(pythonRequestsPath)
			if err != nil {
