...
func TestFixtures(t *testing.T) {
	...
	rDir := filepath.Join(fixturesDir, "r-httr2")
	...
	for _, entry := range entries {
		if entry.IsDir() || filepath.Ext(entry.Name()) != ".R" {
			continue
		}
		base := entry.Name()[:len(entry.Name())-len(".R")]
		t.Run(base, func(t *testing.T) {
			...
			expected, err := os.ReadFile(rPath)
			if err != nil {
