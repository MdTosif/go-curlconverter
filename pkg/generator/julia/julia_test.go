...
func TestFixtures(t *testing.T) {
	...
	juliaDir := filepath.Join(fixturesDir, "julia")

	entries, err := os.ReadDir(juliaDir)
	if err != nil {
		t.Skipf("cannot read fixtures directory: %v", err)
	}

	for _, entry := range entries {
		if entry.IsDir() || filepath.Ext(entry.Name()) != ".jl" {
			continue
		}
		base := entry.Name()[:len(entry.Name())-len(".jl")]
		t.Run(base, func(t *testing.T) {
			cmdPath := filepath.Join(commandDir, base+".sh")
			juliaPath := filepath.Join(juliaDir, entry.Name())

			cmd, err := os.ReadFile(cmdPath)
			if err != nil {
				t.Fatalf("cannot read command file: %v", err)
			}
			expected, err := os.ReadFile(juliaPath)
			if err != nil {
