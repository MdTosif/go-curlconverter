...
func TestFixtures(t *testing.T) {
	...
	swiftDir := filepath.Join(fixturesDir, "swift")

	entries, err := os.ReadDir(swiftDir)
	if err != nil {
	...
	for _, entry := range entries {
		...
		base := entry.Name()[:len(entry.Name())-len(".swift")]
		t.Run(base, func(t *testing.T) {
			cmdPath := filepath.Join(commandDir, base+".sh")
			swiftPath := filepath.Join(swiftDir, entry.Name())

			cmd, err := os.ReadFile(cmdPath)
			if err != nil {
				t.Fatalf("cannot read command file: %v", err)
			}
			expected, err := os.ReadFile(swiftPath)
			if err != nil {
