...
func TestFixtures(t *testing.T) {
	...
	objcDir := filepath.Join(fixturesDir, "objectivec")

	entries, err := os.ReadDir(objcDir)
	if err != nil {
		t.Skipf("cannot read fixtures directory: %v", err)
	}

	for _, entry := range entries {
		if entry.IsDir() || filepath.Ext(entry.Name()) != ".m" {
			continue
		}
		base := entry.Name()[:len(entry.Name())-len(".m")]
		t.Run(base, func(t *testing.T) {
			cmdPath := filepath.Join(commandDir, base+".sh")
			objcPath := filepath.Join(objcDir, entry.Name())

			cmd, err := os.ReadFile(cmdPath)
			if err != nil {
				t.Fatalf("cannot read command file: %v", err)
			}
			expected, err := os.ReadFile(objcPath)
			if err != nil {
