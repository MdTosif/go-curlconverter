...
func TestFixtures(t *testing.T) {
	...
	httpieDir := filepath.Join(fixturesDir, "httpie")

	entries, err := os.ReadDir(httpieDir)
	if err != nil {
	...
	for _, entry := range entries {
		...
		base := entry.Name()[:len(entry.Name())-len(".sh")]
		t.Run(base, func(t *testing.T) {
			cmdPath := filepath.Join(commandDir, base+".sh")
			httpiePath := filepath.Join(httpieDir, entry.Name())

			cmd, err := os.ReadFile(cmdPath)
			if err != nil {
				t.Fatalf("cannot read command file: %v", err)
			}
			expected, err := os.ReadFile(httpiePath)
			if err != nil {
