...
func TestFixtures(t *testing.T) {
	...
	dartDir := filepath.Join(fixturesDir, "dart")

	entries, err := os.ReadDir(dartDir)
	if err != nil {
	...
	for _, entry := range entries {
		...
		base := entry.Name()[:len(entry.Name())-len(".dart")]
		t.Run(base, func(t *testing.T) {
			cmdPath := filepath.Join(commandDir, base+".sh")
			dartPath := filepath.Join(dartDir, entry.Name())

			cmd, err := os.ReadFile(cmdPath)
			if err != nil {
				t.Fatalf("cannot read command file: %v", err)
			}
			expected, err := os.ReadFile(dartPath)
			if err != nil {
