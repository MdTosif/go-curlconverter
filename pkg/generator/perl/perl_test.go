...
func TestFixtures(t *testing.T) {
	...
	perlDir := filepath.Join(fixturesDir, "perl")
	...
	for _, entry := range entries {
		if entry.IsDir() || filepath.Ext(entry.Name()) != ".pl" {
			continue
		}
		base := entry.Name()[:len(entry.Name())-len(".pl")]
		t.Run(base, func(t *testing.T) {
			cmdPath := filepath.Join(commandDir, base+".sh")
			perlPath := filepath.Join(perlDir, entry.Name())

			cmd, err := os.ReadFile(cmdPath)
			if err != nil {
				t.Fatalf("cannot read command file: %v", err)
			}
			expected, err := os.ReadFile(perlPath)
			if err != nil {
