...
func TestFixtures(t *testing.T) {
	...
	csharpDir := filepath.Join(fixturesDir, "csharp")

	entries, err := os.ReadDir(csharpDir)
	if err != nil {
	...
	for _, entry := range entries {
		...
		base := entry.Name()[:len(entry.Name())-len(".cs")]
		t.Run(base, func(t *testing.T) {
			cmdPath := filepath.Join(commandDir, base+".sh")
			csharpPath := filepath.Join(csharpDir, entry.Name())

			cmd, err := os.ReadFile(cmdPath)
			if err != nil {
				t.Fatalf("cannot read command file: %v", err)
			}
			expected, err := os.ReadFile(csharpPath)
			if err != nil {
