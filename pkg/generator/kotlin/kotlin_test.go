...
func TestFixtures(t *testing.T) {
	...
	kotlinDir := filepath.Join(fixturesDir, "kotlin")

	entries, err := os.ReadDir(kotlinDir)
	if err != nil {
	...
	for _, entry := range entries {
		...
		base := entry.Name()[:len(entry.Name())-len(".kt")]
		t.Run(base, func(t *testing.T) {
			cmdPath := filepath.Join(commandDir, base+".sh")
			kotlinPath := filepath.Join(kotlinDir, entry.Name())

			cmd, err := os.ReadFile(cmdPath)
			if err != nil {
				t.Fatalf("cannot read command file: %v", err)
			}
			expected, err := os.ReadFile(kotlinPath)
			if err != nil {
