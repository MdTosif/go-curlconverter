...
func TestFixtures(t *testing.T) {
	...
	rubyDir := filepath.Join(fixturesDir, "ruby-httparty")

	entries, err := os.ReadDir(rubyDir)
	if err != nil {
		t.Skipf("cannot read fixtures directory: %v", err)
	}

	for _, entry := range entries {
		if entry.IsDir() || filepath.Ext(entry.Name()) != ".rb" {
			continue
		}
		base := entry.Name()[:len(entry.Name())-len(".rb")]
		t.Run(base, func(t *testing.T) {
			cmdPath := filepath.Join(commandDir, base+".sh")
			rubyPath := filepath.Join(rubyDir, entry.Name())

			cmd, err := os.ReadFile(cmdPath)
			if err != nil {
				t.Fatalf("cannot read command file: %v", err)
			}
			expected, err := os.ReadFile(rubyPath)
			if err != nil {
