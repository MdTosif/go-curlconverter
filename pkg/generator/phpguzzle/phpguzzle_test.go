...
func TestGenerate(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			contains: "GuzzleHttp\\Client",
		},
		{
			name:     "POST with data",
			curl:     "curl -X POST -d 'key=value' http://example.com",
			contains: "form_params",
		},
	}
...
func TestFixtures(t *testing.T) {
	...
	phpDir := filepath.Join(fixturesDir, "php-guzzle")
	...
	for _, entry := range entries {
		...
		t.Run(base, func(t *testing.T) {
			...
			phpPath := filepath.Join(phpDir, entry.Name())

			cmd, err := os.ReadFile(cmdPath)
			if err != nil {
				t.Fatalf("cannot read command file: %v", err)
			}
			expected, err := os.ReadFile(phpPath)
			if err != nil {
